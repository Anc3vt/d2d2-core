...
/**
 * Экземпляр данного класса представляет собой кнопку прикосновения. 
 * Область экрана, с которой пользователь может взаимодействовать путем
 * нажатия на экран. Является контейнером, соответственно предоставляет возможность
 * быть визуально отображаемым экранным объектом.
 * Чтобы область заработала, ее нужно включить методом setEnable
 * @author ancevt
 *
 */
public class TouchButton extends DisplayObjectContainer {
	 ...
	 * @param width ширина кнопки
	 * @param height высота кнопки
	 * @param enabled указывает, включена ли кнопка. true - включена, false - выключена
	 */
	...
	/**
	 * Событие "пользователь убрал палец с экрана после 
	 * нажатия кнопки"
	 * @param x X-координата прикосновения
	 * @param y Y-координата прикосновения
	 * @param onArea в момент отжатия палец на кнопке - true, иначе - false
	 */
	public void onTouchUp(final int x, final int y, final boolean onArea) {}
	 ...
	 */
	public void onTouchDrag(final int x, final int y) {}
